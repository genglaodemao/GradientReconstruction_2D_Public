function [res,loc] = featureH(image,pN,bestH,bestR,R,step,iffilter,masscut,mask)
...
xyzmax=size(image);
if nargin < 8, masscut=0; end
if nargin < 9, mask=0; end
res=[];
RR=R(1):step:R(2);
l_frno=length(RR);
for i=1:l_frno
...
loc= localmaxH(pN,a,b,RR,sepall,extentall);

if mask>=0
if (numel(loc) == 0 || loc(1) == -1)
    res = [];
    return
elseif length(loc(:,1))<pN
    res=[];
    return
else
   [jk sloc]=sort(loc(:,4));
